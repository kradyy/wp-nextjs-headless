...
const Menu: React.FC<MenuProps> = async ({location}) => {
  ...
  <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
    <a href="#" className="flex items-center">
        <span className="self-center text-2xl font-semibold whitespace-nowrap text-white">{generalSettingsTitle}</span>
    </a>
    ...
    <div className="hidden w-full md:block md:w-auto" id="navbar-default">
        <MenuWalker
            location={location}
            wrapper={(children: any) => <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0">{children}</ul>}
            linkClass="block py-2 pl-3 pr-4 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0"
        />
    </div>       
  </div>
</nav>
